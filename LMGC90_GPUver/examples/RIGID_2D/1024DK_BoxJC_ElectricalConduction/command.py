...
for k in range(1,nb_steps+1,1):
   ...
   i_dkdk  = np.where( inters['inter']=='DKDKx'.encode() )[0]
   i_dkjc  = np.where( inters['inter']=='DKJCx'.encode() )[0]

   UI_inte =  np.zeros([inters.size,2])

   for i_dkdk, i_inter in enumerate(i_dkdk):
      UIC = chipy.mp_solver_GetBrancheValues('DKDKx',i_dkdk+1)
      UI_inte[i_inter,:] = UIC[:2]

   for i_dkjc, i_inter in enumerate(i_dkjc):
