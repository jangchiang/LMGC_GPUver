...
datbox_path = os.path.join(os.getcwd(),'DATBOX')
...
nb_comp_particles, coor = pre.depositInBox3D(radii, Lext, Wext, Hext, seed=seed)
...
for i in range(0,nb_comp_particles,1):
   ...
   body=rigidNconvex(RR=radii[i]-r_p,rr=r_p,n_branch=n_branch, center=coor[3*i : 3*(i + 1)], 
                     model=mod, material=plex, color='BLUEx') 
   body.translate(dx=0.5*(Lext),dy=0.5*(Wext))
   bodies += body
