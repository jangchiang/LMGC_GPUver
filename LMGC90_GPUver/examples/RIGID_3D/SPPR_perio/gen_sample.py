...
rad = 0.1
radii = pre.granulo_Monodisperse(nb_particles, rad)
...
for i in range(0,nb_comp_particles,1):
   if (i%2 == 0):
    ...
    body = pre.rigidPolyhedron(center=coor[3*i : 3*(i + 1)], material=plex, model=mod, color='BLACx',generation_type='regular',nb_vertices=20, radius=radii[i])
   #
   body.translate(dx=lx/2., dy=ly/2.)
   bodies += body
