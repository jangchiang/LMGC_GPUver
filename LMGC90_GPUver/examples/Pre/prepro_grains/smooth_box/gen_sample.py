...
# creation des quatres parois lisses, avec le materiau tdur
down = pre.smoothWall(center=[0.5*lx, -radius_max], theta=0., l=lx, h=2.*radius_max, 
                      nb_polyg=3, model=mod, material=tdur,  color='WALLx')
up = pre.smoothWall(center=[0.5*lx, ly + radius_max], theta=0., l=lx, h=2.*radius_max, 
                    nb_polyg=3,  model=mod, material=tdur, color='WALLx')
left = pre.smoothWall(center=[-radius_max, 0.5*ly], theta=-0.5*numpy.pi, l=ly, h=2.*radius_max, 
                      nb_polyg=2, model=mod, material=tdur, color='WALLx')
right = pre.smoothWall(center=[lx + radius_max, 0.5*ly], theta=0.5*numpy.pi, l=ly, h=2.*radius_max, 
                       nb_polyg=2, model=mod, material=tdur, color='WALLx')
...
svs+=svdkdk
#       - avec les parois
svdkpl = pre.see_table(CorpsCandidat='RBDY2',candidat='DISKx', colorCandidat='BLEUx',
