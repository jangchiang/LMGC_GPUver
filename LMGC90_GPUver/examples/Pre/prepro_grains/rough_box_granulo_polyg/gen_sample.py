...
down = pre.granuloRoughWall(center=[0.5*lx, -radius_max], theta=0., l=lx + 2.*radius_max, 
                      rmin=radius_min, rmax=radius_max, model=mod, material=tdur, color='WALLx', nb_vertex=5)
up = pre.granuloRoughWall(center=[0.5*lx, ly + radius_max], theta=math.pi, l=lx + 2.*radius_max, 
                    rmin=radius_min, rmax=radius_max, model=mod, material=tdur, color='WALLx', nb_vertex=5)
left = pre.granuloRoughWall(center=[-radius_max, 0.5*ly], theta=-0.5*math.pi, l=ly + 2.*radius_max, 
                      rmin=radius_min, rmax=radius_max, model=mod, material=tdur, color='WALLx', nb_vertex=5)
right = pre.granuloRoughWall(center=[lx + radius_max, 0.5*ly], theta=0.5*math.pi, l=ly + 2.*radius_max, 
                       rmin=radius_min, rmax=radius_max, model=mod, material=tdur, color='WALLx', nb_vertex=5)
