...
down = pre.roughWall(center=[0.5*lx, -radius_max], theta=0., l=lx + 2.*radius_max, 
        r=radius_max, model=mod, material=tdur, color='WALLx', nb_vertex=5)
up = pre.roughWall(center=[0.5*lx, ly + radius_max], theta=0., l=lx + 2.*radius_max, 
        r=radius_max, model=mod, material=tdur, color='WALLx', nb_vertex=5)
left = pre.roughWall(center=[-radius_max, 0.5*ly], theta=-0.5*math.pi, l=ly + 2.*radius_max, 
        r=radius_max, model=mod, material=tdur, color='WALLx', nb_vertex=5)
right = pre.roughWall(center=[lx + radius_max, 0.5*ly], theta=0.5*math.pi, l=ly + 2.*radius_max, 
        r=radius_max, model=mod, material=tdur, color='WALLx', nb_vertex=5)
