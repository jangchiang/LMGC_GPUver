...
def generate():
    ...
    [nb_deposit, coor] = pre.depositInBox3D(radii, lx, ly, lz, seed=list(range(33)))
    coor.shape= [coor.size//3,3]

    rigid_set = []
    for i in range(nb_deposit):
      body = pre.rigidPolyhedron(radius=radii[i], nb_vertices=randint(15,35), center=coor[i,:], model=mod, material=plexx, color='BLUEx')
      bodies += body
      if i in [0, 1, 9, 12, 15]:
