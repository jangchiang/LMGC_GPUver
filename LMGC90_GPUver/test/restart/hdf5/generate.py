...
def gen_2D():
    ...
    nb_particles = pre.nbPointsInTriangularLattice2D(nb_ele, nb_layer, 'up') 

    radii = pre.granulo_Random(nb_particles, 0.5, 0.51, 1)
    ...
    [lx, ly] = pre.sizeBoundingBoxTriangularLattice2D(nb_ele, nb_layer, l, 'up')

    coor = pre.triangularLattice2D(nb_ele, nb_layer, l, orientation='up')
    coor.shape = [nb_particles,2]
