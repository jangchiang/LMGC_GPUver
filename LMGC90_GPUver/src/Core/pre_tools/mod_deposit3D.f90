...
contains
   ...
   subroutine new_deposit(nb_particles, given_radii, nb_deposited_particles, deposited_radii, deposited_coor, seed)
      implicit none
      !> number of particles
      ...
      ! optionals :
      !> number of already deposited particles
      integer, intent(in), optional :: nb_deposited_particles
      !> radii of already deposited particles
      real(kind=8), dimension(:), optional :: deposited_radii
      !> coordinates of already deposited particles
      real(kind=8), dimension(:, :), optional :: deposited_coor
      !> an input seed to control the randomness
      integer(kind=4), dimension(:), optional :: seed
      ! local variables 
      ...
      if (present(nb_deposited_particles)) then
         ! on le stocke dans le module
         n_init = nb_deposited_particles
   
         ! on teste la compatibilite des donnees :
         ! * completude des donnees
         if (.not. present(deposited_radii) .or. .not. present(deposited_coor)) then     
            print *,'deposit3D::new_deposit: FATAL ERROR: missing aguments' 
            stop
         endif
         ! * coherence avec les rayons des particules deja deposees :
         if (size(deposited_radii) /= n_init) then
            print *,'deposit3D::new_deposit: FATAL ERROR: non conforming size', & 
                    ' for radii of deposited particles'
            stop
         endif
         ! * coherence avec les coordonnes des particules deja deposees
         if (size(deposited_coor, 1) /= 3 .or. size(deposited_coor, 2) /= n_init) then
            print *,'deposit3D::new_deposit: FATAL ERROR: non conforming size', &
                    ' for coordinates of deposited particles'
            stop
         endif
      ! sinon,
      else
   ...
   subroutine deposit
      ...
      call depot_( n_body, cont, n_verlet, i_list, list )

   end subroutine deposit

   ! procedure qui recupere les coordonnees des particules deposees
   !> Gets the number and coordinates of the deposited particles.
   !> \warning 1) This subroutine do not return the coordinates of the deposited particles, since they are already known...\n
   !>          2) a non deposited particle have null radius and coordinates
   subroutine get_computed_particles(nb_particles, nb_comp_particles, comp_radii, comp_coor)
      ...
      integer, intent(out) :: nb_comp_particles
      !> radii of the deposited particles      
      real(kind=8), dimension(nb_particles), intent(out) :: comp_radii 
      !> coordinates of the deposited particles
      real(kind=8), dimension(3, nb_particles), intent(out) :: comp_coor 

      ! variable locale :
      integer :: i ! indice de boucle
  
      ! on teste la consistance entre le nombre de particules dans la granulo 
      ! et le nombre de particule dont on veut recuperer les coordonnees
      if (nb_particles /= n_body - n_init) then
         print *,'deposit3D::get_coor: FATAL ERROR: non conforming size for coordinates of deposited particles'
         stop
      endif
      ...
      ! on initialise a 0 les rayons et le coordonnees
      comp_radii = 0.d0
      comp_coor = 0.d0   
   ...
   !> computed.
   subroutine depot_( n_body, cont, n_verlet, i_list, list )
