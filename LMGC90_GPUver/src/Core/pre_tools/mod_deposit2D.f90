...
subroutine new_deposit(nb_particles, given_radii, lx, type_potential, nb_big_particles, given_big_radii, given_big_coor)

   implicit none
   
   ! variables d'entree :
   integer, intent(in) :: nb_particles !< number of particles
      ! nombre de particules dans la granulo
   real(kind=8), dimension(nb_particles), intent(in) :: given_radii !< given radii list (i.e. grnulometry)
      ! liste des rayons (i.e. granulo donnee)
   real(kind=8) :: lx !< width of the box
      ! largeur de la boite de depot :
   character(len=*) :: type_potential !< name of the potential to use
      ! type de potentiel a utiliser :
      ! 'gravity' : depot sous gravite
      ! 'wall' : depot depuis les murs vers l'interieur
      ! 'big_particles' : depot autour de grosses particules
 
   ! variables d'entree optionnelles :
   integer, intent(in), optional :: nb_big_particles !< number of big particles, supposed to be already deposited
      ! nombre de grosses particules deja deposees
   real(kind=8), dimension(:), optional :: given_big_radii !< radii of these big particles
      ! rayons des grosses particules
   real(kind=8), dimension(:, :), optional :: given_big_coor !< coordinates of these big particles
   ...
   if (present(nb_big_particles)) then
      ! on le stocke dans le module
      nb_big_bodies = nb_big_particles

      ! on teste la compatibilite des donnees :
      ! * completude des donnees
      if (.not. present(given_big_radii) .or. .not. present(given_big_coor)) then     
         print *,'deposit2D::new_deposit: FATAL ERROR: missing aguments' 
         stop
      endif
      ! * coherence avec les rayons des grosses particules :
      if (size(given_big_radii) /= nb_big_bodies) then
      ...
      if (size(given_big_coor, 1) /= 2 .or. size(given_big_coor, 2) /= nb_big_bodies) then
         print *,'deposit2D::new_deposit: FATAL ERROR: non conforming size', &
      ...
      het_radius(1:nb_big_particles) = given_big_radii
      het_coor(:, 1:nb_big_particles) = given_big_coor
...
!> \warning This subroutine do not return the coordinates of the big particles, since they are already known...
subroutine get_coor(comp_coor)
   
   implicit none

   ! variables de sortie :
   real(kind=8), dimension(2, nb_grains), intent(out) :: comp_coor !< coordinates of the deposited particles
      ! coordonnees des particules deposees
